...
def menu(menuid, **kwargs):
	if menuid == "shutdown":
		return [(_("On/Off timer"), main, "On/Off timer", 49)]
	return []
	
def Plugins(**kwargs):
	list = [PluginDescriptor(name=_("On/Off timer"), description=_("set time to stanby/wakeup"), where = [PluginDescriptor.WHERE_PLUGINMENU], icon="timeoff.png", fnc=main)]
	list.append(PluginDescriptor(name=_("On/Off timer"), description=_("set time to stanby/wakeup"), where = [PluginDescriptor.WHERE_MENU], fnc=menu))
	return list
