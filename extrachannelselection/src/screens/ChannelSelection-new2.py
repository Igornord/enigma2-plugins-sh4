...
from enigma import eServiceReference, eEPGCache, eServiceCenter, eRCInput, eTimer, eDVBDB, iPlayableService, iServiceInformation, getPrevAsciiCode, eEnv, eListboxServiceContent
			...
			if not self.preEnterPath(refstr):
				...
				if justSet:
					...
					if self.mode == 0: # TV mode
						typeslist = ('1:7:1:0:0:0:0:0:0:0:(type == 1) || (type == 22)', '1:7:11:0:0:0:0:0:0:0:(type == 17) || (type == 25) || (type == 134) || (type == 195)')
					else:
					...
					for srvtypes in typeslist:
						...
						if not servicelist is None:
							while True:
								...
								if '(type == 1)' in srvtypes:
									service_type = "SD-%s"%(service_type)
								elif '(type == 17)' in srvtypes:
					...
					if hdref:
						hdref.setName("%s - %s" % ("Sky", _("Subservices") + " (%d)"%(self.getServicesCount(hdref))))
	...
	def channelSelected(self, doClose = True):
		...
		if self.movemode:
			...
			if not root or not (root.flags & eServiceReference.isGroup):
				old = self.session.nav.getCurrentlyPlayingServiceReference()
