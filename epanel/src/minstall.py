...
<screen name="IPKToolsScreen2" position="center,160" size="750,370" title="IPK Tools">
	...
	def clear(self):
		self.iConsole.ePopen("rm /tmp/*.tar.gz /tmp/*.bh.tgz /tmp/*.ipk /tmp/*.nab.tgz")
		...
		if line_old is not None:
			if not line_old[-2]:
				if ".ipk" in line_old[0]:
					...
					self.commamd_line_tar.append(line_old[-1])
				self.status = True
			else:
				if ".ipk" in line_old[0]:
					...
					self.commamd_line_tar.remove(line_old[-1])
				self.status = False
		...
		if line_old is not None:
			if config.plugins.epanel.multifilemode.value is not 'Multi':
				...
				if '.ipk' in self["menu"].getCurrent()[-1]:
					...
					self.commamd_line_tar.append(self["menu"].getCurrent()[-1])
			ipk_list, tar_list, force_string = '', '', ''
			if self.force_install:
				force_string = "-force-overwrite -force-downgrade"
			for item in self.commamd_line_ipk:
				ipk_list += item + ' '
			for item in self.commamd_line_tar:
				tar_list += item + ' '
			if len(ipk_list) > 3 and len(tar_list) > 3:
				self.session.open(Console, title = _("Install packets"), cmdlist = ["sleep 2 && opkg install %s %s && tar -C/ -xzpvf %s" % (force_string, ipk_list, tar_list)])
				ipk_list, tar_list = '', ''
			elif len(ipk_list) > 3:
				self.session.open(Console, title = _("Install packets"), cmdlist = ["sleep 2 && opkg install %s %s" % (force_string, ipk_list)])
			elif len(tar_list) > 3:
				self.session.open(Console,title = _("Install tar.gz, bh.tgz, nab.tgz"), cmdlist = ["tar -C/ -xzpvf %s" % tar_list])
			self.force_install = False
	...
	def nList(self):
		...
		for i in range(len(self.workdir)):
			if pathExists(self.workdir[i]):
				...
				for line in ipklist:
					if ".tar.gz" in line or ".bh.tgz" in line or ".nab.tgz" in line:
						try:
